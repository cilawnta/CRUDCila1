...
export class Tab3Page {
  ...
  async getData(){
    ...
    try{
      const res = await axios.post('http://localhost/ioncrud/get_data_mahasiswaId.php', fd);
      ...
      for(let data of this.mahasiswaData){
        ...
        this.foto_mahasiswa = data.foto_mahasiswa;
      }
  ...
  async updateData(){
    ...
    if(this.foto !=undefined){
      ...
      fd.append('foto_siswa', this.foto_mahasiswa);
    }
    try{
      const res = await axios.post('http://localhost/ioncrud/update_data_mahasiswa.php', fd);
